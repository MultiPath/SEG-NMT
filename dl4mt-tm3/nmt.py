...
def build_model(tparams, inps, options, pix='', return_cost=False, with_compile=False):
    ...
    readout = tensor.tanh(logit_lstm+logit_prev+logit_ctx)
    logit = get_layer('ff')[1](tparams, readout, options,
                               prefix=pix+'ff_logit', activ='linear')
    logit_shp = logit.shape
    probs = tensor.nnet.softmax(logit.reshape([logit_shp[0]*logit_shp[1],
                                               logit_shp[2]]))
    opt_ret['readout'] = readout
    opt_ret['hids']    = proj_h
    opt_ret['probs']   = probs
    opt_ret['logit']   = logit
...
def build_sampler(tparams, options, trng, pix=''):
    ...
    outs = [next_probs, next_sample, next_state, ctxs, atts, readout]
...
def gen_sample_memory(tparams, funcs,
    ...
    hids20, ctxs20, read20 = funcs['crit_xy'](x2, x2_mask, y2, y2_mask)
    ...
    for ii in xrange(maxlen):
        ...
        next_p, next_w, next_state, ctxs, attsum, read1 = ret[0], ret[1], ret[2], ret[3], ret[4], ret[5]
...
def build_networks(options, model=' ', train=True):
    ...
    # get the logit to compute prob
    def compute_logit(readouts1, readouts2, attens, g):
        # readouts1: dec_cu x batch-size x out_dim
        # readouts2: dec_tm x batch-size x out_dim
    ...
    def compute_cost(prob, y, y_mask):

        # normal loss ---> maybe neumerically unstable
        ccost = -tensor.log(compute_prob(prob, y)) * y_mask
        ccost = ccost.sum(0)
        return ccost

    readout1 = ret_xy11['readout']
    readout2 = ret_xy22['readout']
    attens   = outs[2]
    cost     = compute_cost(compute_logit(readout1, readout2, attens, gates), y1, y1_mask)
    ...
    funcs['pred_xy']   = ret_xy11['f_predict']
