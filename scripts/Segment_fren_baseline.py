...
with open(config['valid_datasets'][3], 'rb') as f:
    while True:
        ss = f.readline()
        if ss == '':
            break
        en_match_list.append(ss)
scores = []
...
fr_sort_list = [[] for i in numpy.arange(10)]
fr_match_sort_list = [[] for i in numpy.arange(10)]
en_match_sort_list = [[] for i in numpy.arange(10)]
for i in numpy.arange(len(index)):
    for j in index[i]:
        fr_sort_list[i].append(fr_list[j])
        fr_match_sort_list[i].append(fr_match_list[j])
        en_match_sort_list[i].append(en_match_list[j])
for i in numpy.arange(len(index)):
    with open('/root/workspace/TMNMT/translate/fr_sort_'+ str(i), 'w') as f:
        for j in fr_sort_list[i]:
            f.write(j)
    with open('/root/workspace/TMNMT/translate/fr_match_sort_'+ str(i), 'w') as f:
        for j in fr_match_sort_list[i]:
            f.write(j)
        
    
    with open('/root/workspace/TMNMT/translate/en_match_sort_'+ str(i), 'w') as f:
        for j in en_match_sort_list[i]:
            f.write(j)
...
with open(config['valid_datasets'][1], 'rb') as f:
    while True:
        ...
        en_list.append(ss)
en_trans_list = []
with open('/root/workspace/TMNMT/translate/baseline_fren.valid', 'rb') as f:
    while True:
        ss = f.readline()
        if ss == '':
            break
